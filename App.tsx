...
const App: React.FC = () => {
  ...
  const { audioState, playStory, pauseAudio, stopAudio, skipToSection } = useAudioPlayer();
  const { profiles, activeProfile, createProfile, updateProfile, deleteProfile, setActiveProfile } = useProfiles();
  const { stories, saveStory, deleteStory, rateStory } = useLibrary();
  ...
  const [, setShowSoundscape] = useState(false);
  ...
  // Apply default soundscape from settings (once on mount from persisted defaults)
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- run once; adding deps would restart audio on every settings change
  }, []);
